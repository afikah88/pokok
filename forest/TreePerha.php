...
if (isset($_GET['block']) && $_GET['block'] != '') {
    ...
    if ($result->num_rows > 0) {
        echo "<h3>Tree Count - Block ($block_x, $block_y)</h3>";
        echo "<table>
                <thead>
                    <tr>
                        <th>Species Group</th>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $data[$spgroupName][$diameterRange] = $row['total_trees'];
            $columnTotals[$diameterRange] += $row['total_trees'];
        }

        foreach ($data as $speciesGroup => $ranges) {
            echo "<tr>";
            echo "<td>$speciesGroup</td>";
            
            $rowTotal = 0;
            foreach (['5cm-15cm', '15cm-30cm', '30cm-45cm', '45cm-60cm', '60cm+'] as $range) {
                $trees = isset($ranges[$range]) ? $ranges[$range] : 0;
                $rowTotal += $trees;
                echo "<td>" . ($trees > 0 ? "$trees trees" : "-") . "</td>";
            }
            echo "<td class='total-cell'>" . ($rowTotal > 0 ? "$rowTotal trees" : "-") . "</td>";
            echo "</tr>";
        ...
        foreach ($columnTotals as $total) {
            echo "<td>" . ($total > 0 ? "$total trees" : "-") . "</td>";
        }
        echo "<td class='total-cell'>$grandTotal trees</td>";
        echo "</tr>";
