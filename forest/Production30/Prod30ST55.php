...
if (isset($_GET['block']) && $_GET['block'] != '') {
    ...
    $sql = "
        SELECT
            ...
            CASE 
                ...
                WHEN diameter30 BETWEEN 30 AND 44 THEN '30cm-45cm'
                WHEN diameter30 BETWEEN 45 AND 59 THEN '45cm-60cm'
                ELSE '60cm+'
            END AS diameter_range,
            COUNT(CASE 
                WHEN status IN ('keep', 'victim_crown') 
                AND diameter30 >= 55
                THEN 1 
                ELSE NULL 
            END) AS No,
            ...
            END) AS Vol
        FROM tree_data
        WHERE species_group IN (1, 2, 3, 5)
        AND block_x = '$block_x'
        AND block_y = '$block_y'
    ...
    if ($result->num_rows > 0) {
        echo "<h3>Production Year 30 Cutting 55 - Block ($block_x, $block_y)</h3>";
        echo "<table>
                <thead>
                    <tr>
                        <th>Species Group</th>
        ...
        foreach ($data as $speciesGroup => $ranges) {
            ...
            // Calculate group totals only for production volume > 0
            ...
            foreach (['5cm-15cm', '15cm-30cm', '30cm-45cm', '45cm-60cm', '60cm+'] as $range) {
                $displayNo = isset($ranges[$range]) && $ranges[$range]['Vol'] > 0 ? 
            ...
            echo "<tr>";
            foreach (['5cm-15cm', '15cm-30cm', '30cm-45cm', '45cm-60cm', '60cm+'] as $range) {
                $displayVol = isset($ranges[$range]) && $ranges[$range]['Vol'] > 0 ? 
                    number_format($ranges[$range]['Vol'], 2) . " m³" : "-";
                echo "<td class='volume'>$displayVol</td>";
            }
            echo "<td class='total-cell'>" . ($groupTotals['Vol'] > 0 ? number_format($groupTotals['Vol'], 2) . " m³" : "-") . "</td>";
            echo "</tr>";
        }
        ...
        echo "<td class='total-cell'>" . 
             ($totals['No'] > 0 ? $totals['No'] . " trees<br>" . number_format($totals['Vol'], 2) . " m³" : "-") . 
             "</td>";
        echo "</tr>";
