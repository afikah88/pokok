...
if (isset($_GET['block']) && $_GET['block'] != '') {
    ...
    $sql = "
       SELECT
            CASE
                ...
                WHEN species_group = 4 THEN 'Dipterocarp NonCommercial'
                WHEN species_group = 5 THEN 'NonDipterocarp Commercial'
                WHEN species_group = 6 THEN 'NonDipterocarp NonCommercial'
                WHEN species_group = 7 THEN 'Others'
            END AS spgroup_name,
            CASE 
                WHEN diameter30 BETWEEN 5 AND 15 THEN '5cm-15cm'
                WHEN diameter30 BETWEEN 15 AND 30 THEN '15cm-30cm'
                WHEN diameter30 BETWEEN 30 AND 45 THEN '30cm-45cm'
                WHEN diameter30 BETWEEN 45 AND 60 THEN '45cm-60cm'
                ELSE '60cm+'
            END AS diameter_range,
            COUNT(*) AS No,
            SUM(CASE 
                WHEN status IN ('victim_crown', 'keep') AND diameter30 > 60 THEN volume30
                ELSE 0 
            END) AS Growth
        FROM tree_data
    ...
    if ($result->num_rows > 0) {
        echo "<h3>Growth Year 30 Cutting 60 - Block ($block_x, $block_y)</h3>";
        echo "<table>
                <thead>
                    <tr>
                        <th>Species Group</th>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($row['Growth'] > 0) {
                $totals['No'] += $row['No'];
                $totals['Growth'] += $row['Growth'];
            }
        }

        foreach ($data as $speciesGroup => $ranges) {
            $groupTotals = ['No' => 0, 'Growth' => 0];
            
            foreach ($ranges as $range) {
                if ($range['Growth'] > 0) {
                    $groupTotals['No'] += $range['No'];
                    $groupTotals['Growth'] += $range['Growth'];
                }
            }
            ...
            foreach (['5cm-15cm', '15cm-30cm', '30cm-45cm', '45cm-60cm', '60cm+'] as $range) {
                $displayNo = isset($ranges[$range]) && $ranges[$range]['Growth'] > 0 ? 
                    $ranges[$range]['No'] . " trees" : "-";
                echo "<td>$displayNo</td>";
            }
            echo "<td class='total-cell'>" . ($groupTotals['No'] > 0 ? $groupTotals['No'] . " trees" : "-") . "</td>";
            echo "</tr>";

            // Display growth
            echo "<tr>";
            foreach (['5cm-15cm', '15cm-30cm', '30cm-45cm', '45cm-60cm', '60cm+'] as $range) {
                $displayGrowth = isset($ranges[$range]) && $ranges[$range]['Growth'] > 0 ? 
                    number_format($ranges[$range]['Growth'], 2) . " cm" : "-";
                echo "<td class='growth'>$displayGrowth</td>";
            }
            echo "<td class='total-cell'>" . ($groupTotals['Growth'] > 0 ? number_format($groupTotals['Growth'], 2) . " cm" : "-") . "</td>";
            echo "</tr>";
        }
        ...
        echo "<td class='total-cell'>" . 
             ($totals['No'] > 0 ? $totals['No'] . " trees<br>" . number_format($totals['Growth'], 2) . " cm" : "-") . 
             "</td>";
        echo "</tr>";
